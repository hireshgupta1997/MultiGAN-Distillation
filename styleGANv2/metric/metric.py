...
def get_fake_images_and_acts_I2I(args, Encoder, Align, inception, g_running, code_size, step, alpha, sample_num=5000,
                                 batch_size=16, source_loader=None):
    ...
    for gen_in in loader:
        ...
        with torch.no_grad():
            ...
            fake_image = g_running(
                gen_in, step=step, alpha=alpha, E1_output_feat=True, L_feat=L_feat, RESOLUTION=args.RESOLUTION,
                E1_fea_w=args.E1_fea_w, G_fea_w=args.G_fea_w
            )
...
def get_fake_images_and_acts_only_for_fake(inception, g_running, discriminator, align, code_size, step, alpha,
                                           sample_num=5000, batch_size=16, encoder=None, E1_output_feat=False,
                                           L_feat=None, RESOLUTION=None, E1_fea_w=None):
    dataset = TensorDataset(torch.randn(sample_num, code_size))
    ...
    for gen_in in loader:
        ...
        with torch.no_grad():
            ...
            images_reco_4 = g_running(noise_style, noise=noise, step=step, alpha=alpha, E1_output_feat=True,
                                      L_feat=D_feature, RESOLUTION=[64, 32, 16, 8, 4], E1_fea_w=E1_fea_w,
                                      G_fea_w=G_fea_w)
            out = inception(fake_image)
            out = out[0].squeeze(-1).squeeze(-1)
...
def get_fake_images_and_acts(inception, g_running, miner, miner_semantic, code_size, sample_num=5000, batch_size=16,
                             device=None):
    ...
    for gen_in in loader:
        ...
        with torch.no_grad():
            if miner is not None:
                fake_image = g_running(miner(gen_in), miner_semantic=miner_semantic)[
                    0]  # output is image and latent feature
            else:

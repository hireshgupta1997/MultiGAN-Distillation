...
def train(args, gen_target, g_optim, g_emas, inception, num_ws=1000, ckpt_target_path=None):
    ...
    scales = nn.Parameter(torch.randn(num_gens, args.latent).to(device))
    shifts = nn.Parameter(torch.randn(num_gens, args.latent).to(device))
    ...
    if ckpt_target_path is not None:
        ...
        scales.data = ckpt['scales'].data.to(device)
        shifts.data = ckpt['shifts'].data.to(device)
    ...
    for i in pbar:
        ...
        if args.wandb:
            wandb.log({
                ...
                "g_loss_l2": g_loss_l2,
                f"g{gen_ind}_loss_perceptual": g_loss_perceptual,
                f"g{gen_ind}_loss_l2": g_loss_l2,
        ...
        if ((i-gen_ind) % 100 == 0): # Visualize every 100 iterations
            ...
            utils.save_image(
                fake_img,
                '%s/%s.png' % (os.path.join(args.output_dir, 'samples'), str(i).zfill(6) + f"_pred_{gen_ind}"),
                nrow=int(args.n_sample ** 0.5),
                normalize=True,
                range=(-1, 1),
            )
...
def get_args():
    ...
    assert args.ckpt_2 is not None

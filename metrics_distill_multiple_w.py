...
def eval(args, gen_target, g_emas, num_ws, ckpt_target_path, z_path=None):
    for g_ema in g_emas:
        g_ema.eval()
    ...
    shifts = ckpt['shifts'].to(device)

    truncation_latents = []
    for g_ema in g_emas:
        truncation_latents.append(g_ema_1.mean_latent(n_latent=1024))
    ...
    for gen_ind in range(len(g_emas)):
        ...
        for i in range(num_batches):
            ...
            with torch.no_grad():
                real_img, real_w = g_emas[gen_ind]([sample_z], miner=None, miner_semantic=None, return_latents=True,
                                                   truncation=0.5, truncation_latent=truncation_latents[gen_ind])  # (n_sample, 3, H, W)

            # 3 (b). Get fake image from target GAN
            gen_target.input.input.requires_grad_(False)
            gen_target.input.input.data = g_emas[gen_ind].input.input.data
            input_w = real_w[:, 0, :] * scales[gen_ind].unsqueeze(0) + shifts[gen_ind].unsqueeze(0)
            ...
            utils.save_image(
                real_img,
                '%s/%s.png' % (os.path.join(args.output_dir), str(i).zfill(6) + f"_real_{gen_ind}"),
            ...
            utils.save_image(
                fake_img,
                '%s/%s.png' % (os.path.join(args.output_dir), str(i).zfill(6) + f"_pred_{gen_ind}"),
                nrow=int(args.batch ** 0.5),
                normalize=True,
                range=(-1, 1),
            )
        ...
        with open(metrics_path, 'a') as f:
            f.writelines(f'gen_ind:{gen_ind}\tpsnr:{mean_psnr:.2f}\tssim:{mean_ssim:.2f}\n')
...
if __name__ == "__main__":
    ...
    eval(args, gen_target=gen_target, g_emas=[g_ema_1, g_ema_2], ckpt_target_path=args.ckpt_target, num_ws=args.num_ws, z_path=args.z_path)